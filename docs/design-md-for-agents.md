...
Apps built from this scaffold ship with a **canonical house style**: root **`DESIGN.md`** (Devscrolls UI foundation) and the **`devscrolls`** DaisyUI theme in **`src/styles/app.css`**. Agents must follow **`.cursor/rules/follow-devscrolls-ui-foundation.mdc`** for all routine UI unless the product owner opted out.
