...
1. **Default:** follow repo-root **`DESIGN.md`** (Devscrolls foundation, brand personality, **scaffold-first** policy) and **`src/styles/app.css`** theme `devscrolls` — see **`.cursor/rules/follow-devscrolls-ui-foundation.mdc`**. **Fleet-wide** token/theme edits: [`fleet-ui-evolve-in-scaffold-first.md`](./fleet-ui-evolve-in-scaffold-first.md).
