...
- **Snippet or summary:** Worker entry in `src/index.ts`; `wrangler.jsonc` sets `main` and optional bindings. Run `npm run dev` / `npm run deploy`; after binding edits run `npm run cf-typegen`. ASSETS fallback lives in `handle()` next to `app.fetch`.  
